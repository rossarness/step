...
class AttributesBox(MyGrid):
    ...
    def load_character(self):
        ...
        self.main_attributes.extend(
            (self.zw, self.inu, self.czj, self.um, self.kon, self.dh)
        )
                    ...
                    if hasattr(a, "focus"):
                        if hasattr(a, "name"):
                            if a.name is not "character_name":
                                new_value = db.get_attribute(
                                    self.root_app.character, a.name
                                )
    ...
    def update_health(self, hp_value):
        ...
        for hp_item in self.current_hp:
            if (
                hp_item.text == " "
                or int(hp_item.text) > hp_value
                or int(hp_item.text) == hp_max
            ):
                hp_item.text = str(hp_value)
